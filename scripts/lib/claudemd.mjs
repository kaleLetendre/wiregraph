...
This graph may include **linked members** — external directories joined only by a wire (e.g. a client repo and the server it calls over HTTP), connected with \`/wiregraph-link\`. \`graph_stats\` groups them under **Own root** vs **Linked**, and a cross-member seam shows up in \`trace_contract\`/\`path_between\` exactly like a local one — so a "how does A reach B" question spans linked repos without you opening either tree. \`/wiregraph-list\` shows the members; \`/wiregraph-unlink\` removes one.
